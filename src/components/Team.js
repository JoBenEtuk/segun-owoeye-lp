...
import { useOnScreen, useWindowSize, useMediaQuery } from "../hooks"
...
const Hero = () => {
    ...
    useEffect(() => {
        const size = window.scrollX;
        const carousel = document.querySelector('#team-carousel')
        const leftMargin = size + carousel.getBoundingClientRect().left
        const rightMargin = size + carousel.getBoundingClientRect().right

        if (isTablet) {
            document.querySelector('.slick-prev').style.left = `${leftMargin - 20}px`
            document.querySelector('.slick-next').style.left = `${rightMargin + 3}px`
        } else {
            document.querySelector('.slick-prev').style.left = `${leftMargin - 7}px`
            document.querySelector('.slick-next').style.left = `${rightMargin - 13}px`
        }
